...
@TRANSFORMS.register_module()
class UniformSample1(BaseTransform):
    """Uniformly sample frames from the video with enhanced strategy.

    Enhanced sampling strategy:
    1. From the existing frame indices list, select (num_clips + 1) indices from the middle portion
    2. These indices divide the middle portion into num_clips intervals
    3. Randomly select one index from each interval
    4. Add these new indices to the existing sampled indices list

    Required keys:

        - total_frames
        - start_index
        - frame_inds (existing frame indices)

    Added keys:

        - frame_inds (updated with new indices)
        - clip_len
        - frame_interval
        - num_clips
    ...
    Args:
        ...
        test_mode (bool): Store True when building test or validation dataset.
            Defaults to False.
        middle_portion (float): Portion of the middle part to sample from.
            Defaults to 0.5.
    """
    ...
    def __init__(self,
                 ...
                 num_clips: int=1,
                 test_mode: bool = False,
                 middle_portion: float = 0.5) -> None:

        self.clip_len = clip_len
        self.new_num_clips = new_num_clips
        self.test_mode = test_mode
        self.middle_portion = middle_portion
        self.num_clips = num_clips

    def _get_sample_clips(self, existing_inds: np.ndarray) -> np.ndarray:
        """Enhanced sampling strategy from existing indices.

        Args:
            existing_inds (np.ndarray): Existing frame indices.

        Returns:
            seq (np.ndarray): Newly sampled frame indices.
        """
        if len(existing_inds) == 0:
            return np.array([], dtype=np.int32)

        # Sort the existing indices
        sorted_inds = np.sort(existing_inds)

        # Calculate the middle portion of the existing indices
        n = len(sorted_inds)
        middle_start = int((1 - self.middle_portion) * n / 2)
        middle_end = n - middle_start + 1

        # Extract the middle portion
        middle_inds = sorted_inds[middle_start:middle_end]

        # Select num_clips + 1 points from the middle portion
        if len(middle_inds) < self.new_num_clips + 1:
            # If not enough points, use all available points
            seg_points = middle_inds
        else:
            # Evenly select num_clips + 1 points
            indices = np.linspace(0, len(middle_inds) - 1, self.new_num_clips + 1, dtype=int)
            seg_points = middle_inds[indices]

        inds = []
        # For each interval, randomly select one frame
        for i in range(len(seg_points) - 1):
            start = seg_points[i]
            end = seg_points[i + 1]

            if start == end:
                # If start and end are the same, just use that value
                inds.append(start)
            else:
                # Randomly select one index from the interval
                inds.append(np.random.randint(start, end))

        return np.array(inds)

    def transform(self, results: Dict) -> Dict:
        """Perform the Enhanced Uniform Sampling.
        ...
        existing_inds = results.get('frame_inds', np.array([], dtype=np.int32))

        # Sample new indices using the enhanced strategy
        new_inds = self._get_sample_clips(existing_inds)

        # Combine existing and new indices
        all_inds = np.concatenate([existing_inds, new_inds])
        all_inds = np.sort(all_inds)  # Ensure indices are in order

        # Add start index offset if needed
        if 'start_index' in results:
            start_index = results['start_index']
            all_inds = all_inds + start_index

        results['frame_inds'] = all_inds.astype(np.int32)
        results['clip_len'] = self.clip_len
        results['frame_interval'] = None
        results['num_clips'] = self.num_clips
        return results
    ...
    def __repr__(self) -> str:
        repr_str = (f'{self.__class__.__name__}('
                    ...
                    f'num_clips={self.num_clips}, '
                    f'test_mode={self.test_mode}, '
                    f'middle_portion={self.middle_portion})')
        return repr_str
