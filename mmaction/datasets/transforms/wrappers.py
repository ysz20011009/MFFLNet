...
class PytorchVideoWrapper(BaseTransform):
    ...
    #
    # def transform(self, results):
    #     """Perform PytorchVideoTrans augmentations with double application and fusion.
    #
    #     Args:
    #         results (dict): The resulting dict to be modified and passed
    #             to the next transform in pipeline.
    ...
    #
    #     # 内部函数：应用一次变换
    #     def apply_transform(imgs_list):
    #         if self.op in ('AugMix', 'RandAugment'):
    #             # list[ndarray(h, w, 3)] -> torch.tensor(t, c, h, w)
    #             imgs = [x.transpose(2, 0, 1) for x in imgs_list]
    #             imgs = to_tensor(np.stack(imgs))
    #             imgs = self.trans(imgs).data.numpy()
    #             imgs[imgs > 255] = 255
    #             imgs[imgs < 0] = 0
    #             imgs = imgs.astype(np.uint8)
    #             return [x.transpose(1, 2, 0) for x in imgs]
    #         else:
    #             # list[ndarray(h, w, 3)] -> torch.tensor(c, t, h, w)
    #             imgs = np.stack(imgs_list).transpose(3, 0, 1, 2)  # (c, t, h, w)
    #             imgs = to_tensor((imgs / 255.).astype(np.float32))
    #             imgs = self.trans(imgs).data.numpy()
    #             imgs = imgs * 255
    #             imgs[imgs > 255] = 255
    #             imgs[imgs < 0] = 0
    #             imgs = imgs.astype(np.uint8)
    #             # (c, t, h, w) -> (t, h, w, c) -> list[ndarray(h, w, 3)]
    #             return [x for x in imgs.transpose(1, 2, 3, 0)]
    #
    #     # 应用两次独立的变换
    #     transformed1 = apply_transform(results['imgs'])
    #     transformed2 = apply_transform(results['imgs'])
    #
    #     # 融合两次变换结果（平均融合）
    #     fused_imgs = []
    #     for img1, img2 in zip(transformed1, transformed2):
    #         fused = img1.astype(np.float32) + img2.astype(np.float32)
    #         fused = fused/ 2.0
    #         fused = np.clip(fused, 0, 255).astype(np.uint8)
    #         fused_imgs.append(fused)
    #
    #         results['imgs'] = fused_imgs
    #     return results
    ##DAFT
    # """Dual Augmentation Fusion Layer (DAFL) as described in the paper.
    #
    # This layer applies:
    # 1. Two independent random augmentations to the input frames
    # 2. Temporal fusion to each augmented sample using either linear blending or mask blending
    # 3. Combines both fused samples into the results['imgs'] list
    #
    # Args:
    #     op (str): The name of the pytorchvideo transformation.
    # """
    ...
    #         import torch
    #     except ImportError:
    #         raise RuntimeError('Install pytorchvideo to use this transform')
    #     if digit_version(torch.__version__) < digit_version('1.8.0'):
    #         raise RuntimeError(
    #             'The version of PyTorch should be at least 1.8.0')
    #
    ...
    #                                     'RandomShortSideScale')
    #     assert op in supported_pytorchvideo_trans,\
    #         f'PytorchVideo Transform {op} is not supported'
    #
    #     self.trans = trans(**kwargs)
    #     self.op = op
    #
    # def transform(self, results):
    #     """Apply DAFL: generate two augmented samples, apply temporal fusion,
    #     and combine both into results['imgs'].
    #     """
    #     assert 'imgs' in results
    #     assert 'clip_len' in results, "clip_len must be present in results"
    #
    #     # Get original frames and dimensions
    #     original_imgs = results['imgs']
    #     T = len(original_imgs)  # Number of frames
    #     H, W, C = original_imgs[0].shape  # Frame dimensions
    #
    #     # Prepare input tensor for augmentation
    #     if self.op in ('AugMix', 'RandAugment'):
    #         # list[ndarray(h, w, 3)] -> torch.tensor(t, c, h, w)
    #         imgs = [x.transpose(2, 0, 1) for x in original_imgs]
    #         input_tensor = to_tensor(np.stack(imgs))
    #     else:
    #         # list[ndarray(h, w, 3)] -> torch.tensor(c, t, h, w)
    #         # uint8 -> float32
    #         input_tensor = to_tensor((np.stack(original_imgs).transpose(3, 0, 1, 2) /
    #                                  255.).astype(np.float32))
    #
    #     # Generate two independent augmented versions
    #     augmented1 = self.trans(input_tensor.clone())
    #     augmented2 = self.trans(input_tensor.clone())
    #
    #     # Convert back to numpy and process based on augmentation type
    #     def process_augmented(aug_tensor):
    #         """Convert augmented tensor back to numpy frames."""
    #         aug_np = aug_tensor.data.numpy()
    #
    #         if self.op in ('AugMix', 'RandAugment'):
    #             aug_np[aug_np > 255] = 255
    #             aug_np[aug_np < 0] = 0
    #             aug_np = aug_np.astype(np.uint8)
    #             # torch.tensor(t, c, h, w) -> list[ndarray(h, w, 3)]
    #             return [x.transpose(1, 2, 0) for x in aug_np]
    #         else:
    #             # float32 -> uint8
    #             aug_np = aug_np * 255
    #             aug_np[aug_np > 255] = 255
    ...
    #             return [x for x in aug_np.transpose(1, 2, 3, 0)]
    #
    #     # Get two sets of augmented frames
    #     frames1 = process_augmented(augmented1)
    #     frames2 = process_augmented(augmented2)
    #
    #     # Apply temporal fusion to each augmented sample
    #     def apply_temporal_fusion(frames):
    #         """Apply temporal fusion to a sequence of frames."""
    #         T = len(frames)
    #         fused_frames = []
    #
    #         # Randomly choose fusion method (0 for linear, 1 for mask) - same for all frames
    #         vartheta = random.randint(0, 1)
    #
    #         for t in range(T):
    #             # Get corresponding symmetric frame
    #             sym_idx = T - t - 1  # 0-indexed symmetric index
    #
    #             if vartheta == 0:
    #                 # Linear blending: λIt + (1-λ)IT-t+1
    #                 lambda_val = random.uniform(0, 1)  # Blending coefficient
    #                 fused_frame = lambda_val * frames[t].astype(np.float32) + \
    #                              ((1 - lambda_val) * frames[sym_idx].astype(np.float32))
    #             else:
    #                 # Mask blending: M⊙It + (1-M)⊙IT-t+1
    #                 # Generate random rectangular mask
    #                 mask_height = random.randint(int(H*0.1), int(H*0.5))
    #                 mask_width = random.randint(int(W*0.1), int(W*0.5))
    #
    #                 # Random position
    #                 top = random.randint(0, H - mask_height)
    #                 left = random.randint(0, W - mask_width)
    #
    #                 # Create mask (1 outside rectangle, 0 inside)
    #                 M = np.ones((H, W, C), dtype=np.float32)
    #                 M[top:top+mask_height, left:left+mask_width] = 0
    #
    #                 # Apply mask blending
    #                 fused_frame = (M * frames[t].astype(np.float32)) + \
    #                              ((1 - M) * frames[sym_idx].astype(np.float32))
    #
    ...
    #         return fused_frames
    #
    #     # Apply temporal fusion to both augmented samples
    #     fused_frames1 = apply_temporal_fusion(frames1)
    #     fused_frames2 = apply_temporal_fusion(frames2)
    #
    #     # Combine both fused samples into one frame sequence
    #     combined_frames = fused_frames1 + fused_frames2
    #
    #     # Update results with the combined frames
    #     results['imgs'] = combined_frames
    #     results['clip_len'] = len(combined_frames)  # Now 2T
    #
    #     # Also save the original frame count for reference
    #     results['original_clip_len'] = T
    #
    #     return results
    ###yuanshi
    """PytorchVideoTrans Augmentations, under pytorchvideo.transforms.
